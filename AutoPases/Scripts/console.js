(function($, undefined){
    window.kendoConsole = {
        log: function (message, isError, container, IsSpecialEvent) {
            ...
            if (!lastContainer.length || message !== lastMessage) {
                $("<div" + (isError ? " class='error'" : IsSpecialEvent ? " class='specialEvent'" : "") + "/>")
                    .css({
                        marginTop: -24,
        ...
        specialEvent: function (message) {
            this.log(message, false, null, true);
        }
    };
})(jQuery);
